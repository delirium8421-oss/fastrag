...
# INSTRUCTIONS
1. **ENTITY IDENTIFICATION**: Identify and meticulously extract all entities mentioned in the document that belong to the provided ENTITY TYPES. For each entity, provide a MANDATORY concise description capturing its key features within the document's context. Use singular entity names and split compound concepts when necessary for clarity. **EVERY ENTITY MUST HAVE A NON-EMPTY DESCRIPTION.**
2. **RELATIONSHIP DISCOVERY**: Identify and describe ALL relationships between the extracted entities. Resolve pronouns to entity names for clarity. Ensure relationship descriptions clearly explain the connection between entities. **EVERY RELATIONSHIP MUST HAVE A NON-EMPTY DESCRIPTION.**
...
4. **OUTPUT FORMAT: STRICTLY VALID JSON**: Output MUST be in strictly valid JSON format. Adhere to ALL standard JSON rules. The JSON MUST contain three top-level lists: "entities", "relationships", and "other_relationships". Each list item must be a JSON object with REQUIRED fields:
   - For entities: "name" (string), "type" (string), "desc" (string - MANDATORY, non-empty)
   - For relationships: "source" (string), "target" (string), "desc" (string - MANDATORY, non-empty)
   - All fields must be JSON strings enclosed in DOUBLE QUOTES ONLY. **ABSOLUTELY NO SINGLE QUOTES, BRACKETS FOR STRINGS, TRAILING COMMAS, OR MARKDOWN FORMATTING (like triple backticks) ARE PERMITTED.** Invalid JSON output is unacceptable.

# CRITICAL VALIDATION RULES
✓ Every entity MUST have a "desc" field with a non-empty string value
✓ Every relationship MUST have a "desc" field with a non-empty string value
✓ If you cannot provide a description for an entity/relationship, DO NOT include it in the output
